...
public class MapGenerate {
    public static void generateMap(int mapx, int mapy, int switch_trap, int holes, int box_target, int player) {
        try {
    ...
    public static boolean szomszed_byID(int[][] v, int x, int y, int ItemID) {
        if (v[x - 1][y + 1] != ItemID && v[x][y + 1] != ItemID && v[x + 1][y + 1] != ItemID && v[x + 1][y] != ItemID && v[x + 1][y - 1] != ItemID && v[x][y - 1] != ItemID && v[x - 1][y - 1] != ItemID && v[x - 1][y] != ItemID)
            return true;
        else return false;
    }
